/*
 * Qiskit Runtime API
 *
 * The Qiskit Runtime API description
 *
 * The version of the OpenAPI document: 0.25.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// EstimatorV2InputOptionsExecution : Execution options
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct EstimatorV2InputOptionsExecution {
    /// Whether to reset the qubits to the ground state for each shot
    #[serde(rename = "init_qubits", skip_serializing_if = "Option::is_none")]
    pub init_qubits: Option<bool>,
    /// The delay between a measurement and the subsequent quantum circuit
    #[serde(rename = "rep_delay", skip_serializing_if = "Option::is_none")]
    pub rep_delay: Option<f64>,
}

impl EstimatorV2InputOptionsExecution {
    /// Execution options
    pub fn new() -> EstimatorV2InputOptionsExecution {
        EstimatorV2InputOptionsExecution {
            init_qubits: None,
            rep_delay: None,
        }
    }
}
